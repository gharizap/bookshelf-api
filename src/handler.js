...
const getAllBooksHandler = (request, h) => {
  ...
  const allBookFiltered = books
    .filter((book) => {
      ...
      if (reading !== undefined) {
        matchReading = book.reading == reading;
      }

      if (finished !== undefined) {
        matchFinished = book.finished == finished;
      }
  ...
  if (allBookFiltered.length > 0) {
    ...
    const allBook = books.map(({ id, name, publisher }) => ({
      id,
      name,
      publisher,
    }));
